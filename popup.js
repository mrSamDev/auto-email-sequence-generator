document.addEventListener("DOMContentLoaded", () => {
  ...
  function addCredentialToList(email, password, dateCreated) {
    ...
    <div class="flex flex-row md:flex-col md:space-y-1 space-x-1 md:space-x-0">
      ...
      <button class="copy-btn fill-email-btn text-white font-bold py-1 px-2 text-xs rounded" data-credential="${email}" data-type="email">
        Fill Email
      </button>
      <button class="copy-btn fill-password-btn text-white font-bold py-1 px-2 text-xs rounded" data-credential="${password}" data-type="password">
        Fill Password
      </button>
    </div>
  `;
    ...
    li.querySelector(".fill-email-btn").addEventListener("click", function () {
      fillInput("fill-email", this.getAttribute("data-credential"));
    });

    li.querySelector(".fill-password-btn").addEventListener("click", function () {
      fillInput("fill-password", this.getAttribute("data-credential"));
    });
  }
  function fillInput(action, value) {
